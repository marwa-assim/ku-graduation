...
export async function POST(req:Request){
 ...
 for(const person of people??[]){if(!person.email){failed++;errors.push(`${person.full_name}: no email`);continue}try{
  ...
  const rawBody=replacePlaceholders(design.email_body||"Dear {{name}},<br/><br/>Please find your personalized invitation attached.<br/><br/>Date: {{date}}<br/>Time: {{time}}<br/>Venue: {{venue}}<br/>Directions: {{gps_link}}",values);
  const emailFont=String(design.email_font_family||"Arial").replace(/[^a-zA-Z0-9 ,"'-]/g,"");const emailSize=Math.min(32,Math.max(10,Number(design.email_font_size||16)));const emailColor=/^#[0-9A-Fa-f]{6}$/.test(String(design.email_text_color||""))?design.email_text_color:"#111827";const safeBody=rawBody.replace(/<script[\s\S]*?<\/script>/gi,"").replace(/on\w+\s*=\s*["'][^"']*["']/gi,"");const html=`<div style="font-family:${escapeHtml(emailFont)},sans-serif;font-size:${emailSize}px;color:${emailColor};line-height:1.6">${safeBody}<p><a href="${escapeHtml(gpsLink)}">Open GPS directions</a></p></div>`;
