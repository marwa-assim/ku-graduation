...
export function TicketImageActions({ticketId,personId,fileName,compact=false,allowShare=true}:Props){
 ...
 const email=async()=>{setBusy("email");setMsg("");try{const{f,m}=await ensure();downloadFile(f);window.location.href=`mailto:?subject=${encodeURIComponent(m.subject)}&body=${encodeURIComponent(m.body+"\n\nPlease attach the downloaded ticket image: "+f.name)}`;setMsg("Email draft opened. Attach the downloaded ticket image, choose the recipient, and send.")}catch(e:any){setMsg(e?.message||"Could not open the email draft")}finally{setBusy(null)}};
