...
declare
 ...
 v_free_quota int:=0; v_paid_quota int:=0; v_hold int:=15; v_paid_total numeric(10,3):=0; v_effective_price numeric(10,3):=0; v_paid_open timestamptz; v_paid_close timestamptz; v_extra_open timestamptz; v_extra_close timestamptz; v_normal_price numeric(10,3):=0; v_extra_price numeric(10,3):=0;
...
begin
 ...
 into v_org,v_free_quota,v_paid_quota,v_hold,v_paid_open,v_paid_close,v_extra_open,v_extra_close,v_normal_price,v_extra_price from public.events
 ...
 loop
  ...
  if v_seat.seat_type='free_guest' then v_free_requested:=v_free_requested+1;v_free_ids:=array_append(v_free_ids,v_seat.id);
   ...
   if v_extra_open is not null and now()>=v_extra_open and (v_extra_close is null or now()<=v_extra_close) then v_effective_price:=coalesce(v_extra_price,v_normal_price,0);
   ...
   v_paid_requested:=v_paid_requested+1;v_paid_ids:=array_append(v_paid_ids,v_seat.id);v_paid_total:=v_paid_total+v_effective_price;end if;
 end loop;
 ...
 if v_paid_requested>0 then
  ...
  insert into public.booking_seats(booking_id,seat_id,price_bhd) select v_paid_booking,id,v_effective_price from public.seats where id=any(v_paid_ids);
