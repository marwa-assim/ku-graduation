...
-- Expired holds must release both the physical seat and the unique booking_seats row.
create or replace function public.release_expired_bookings()
returns integer language plpgsql security definer set search_path=public as $$
declare affected integer:=0; expired_ids uuid[];
begin
 select coalesce(array_agg(id),'{}'::uuid[]) into expired_ids
 from public.bookings where status='held' and expires_at<now();
 if cardinality(expired_ids)=0 then return 0; end if;
 update public.seats s set status='available',held_until=null
 where exists(select 1 from public.booking_seats bs where bs.booking_id=any(expired_ids) and bs.seat_id=s.id);
 get diagnostics affected=row_count;
 delete from public.tickets where booking_id=any(expired_ids) and status<>'used';
 delete from public.booking_seats where booking_id=any(expired_ids);
 update public.bookings set status='expired' where id=any(expired_ids);
 return affected;
end $$;
grant execute on function public.release_expired_bookings() to authenticated;

-- Clean stale unique seat links before a new reservation, while preserving active bookings.
...
begin
 ...
 -- Rows left by cancelled, failed or expired bookings are historical debris and block the unique seat constraint.
 delete from public.booking_seats bs using public.bookings b
 where bs.booking_id=b.id and bs.seat_id=any(p_seat_ids)
 and b.status in ('expired','cancelled','failed','refunded');

 for v_seat in select id,seat_type,coalesce(price_bhd,0) price_bhd,status
  from public.seats where id=any(p_seat_ids) and event_id=p_event_id order by id for update
 loop
  ...
  if exists(select 1 from public.booking_seats bs join public.bookings b on b.id=bs.booking_id
    where bs.seat_id=v_seat.id and b.status in ('confirmed','held','paid')) then
    raise exception 'One or more seats are already linked to an active booking';
  end if;
 ...
 into v_free_existing,v_paid_existing from public.tickets t join public.seats s on s.id=t.seat_id
 ...
 if v_free_requested>0 then
  ...
  select v_org,p_event_id,p_user_id,v_person,id,v_free_booking,'valid' from public.seats where id=any(v_free_ids) on conflict do nothing;
  update public.seats set status='booked',held_until=null where id=any(v_free_ids);
 end if;
 if v_paid_requested>0 then
  insert into public.bookings(organization_id,event_id,user_id,person_id,status,total_bhd,expires_at,assigned_by_admin)
  values(v_org,p_event_id,p_user_id,v_person,'held',v_paid_total,now()+make_interval(mins=>greatest(5,coalesce(v_hold,15))),false) returning id into v_paid_booking;
