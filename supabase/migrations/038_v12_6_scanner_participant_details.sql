-- v12.6 scanner response enrichment for clear gate verification details
begin;
...
begin
 ...
 select * into existing_scan from public.entry_scans where ticket_id=t.id and result='accepted' limit 1;
 if found then raise exception 'Ticket has already been used'; end if;
 select * into person_record from public.people_directory
 where organization_id=public.current_org() and (id=t.person_id or profile_id=t.user_id)
 order by case when id=t.person_id then 0 else 1 end limit 1 for update;
 ...
 if s.seat_type in ('graduate','staff') and person_record.id is not null then
  update public.people_directory set arrival_status='entered',arrived_at=accepted_at,arrived_by=p_scanned_by where id=person_record.id;
 end if;
 return jsonb_build_object(
  ...
  'referenceNumber',person_record.reference_number,
  'college',person_record.college,
  'program',person_record.program,
  ...
  'ticketType',s.seat_type,'seatCode',coalesce(s.label,s.code),'arrivalStatus','entered','arrivedAt',accepted_at
 );
