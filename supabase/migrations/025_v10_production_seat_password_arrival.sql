-- v10.0 production stabilization: deterministic seat codes, safe regeneration, arrival tracking
begin;
...
-- Make the seat section contract explicit and compatible with every supported zone.
...
declare
 ...
 keep_id uuid;
begin
 if public.app_current_role() not in ('admin') then
 ...
 prefix:=upper(coalesce(nullif(regexp_replace(trim(coalesce(z.seat_prefix,'')),'[^A-Za-z0-9]','','g'),''),'S'));
 ...
 -- Remove stale aisle rows and duplicate available rows at the same physical position.
 delete from public.seats where zone_id=z.id and coalesce(is_aisle,false)=true;
 with ranked as (
  select id,row_number() over(partition by zone_id,row_number,column_number order by
   case when status='booked'::public.seat_status then 0 when status='held'::public.seat_status then 1 else 2 end,id) rn
  from public.seats where zone_id=z.id and coalesce(is_aisle,false)=false
 )
 delete from public.seats s using ranked rnk
 where s.id=rnk.id and rnk.rn>1 and s.status='available'::public.seat_status;

 -- Move current codes out of the way first so the event/code unique key can never collide.
 update public.seats
 set code='TMP-'||id::text,label='TMP-'||id::text
 where zone_id=z.id and coalesce(is_aisle,false)=false;

 -- Remove available seats outside the configured rectangle. Booked/held seats are preserved.
 ...
 for r in 1..z.rows_count loop
  for c in 1..z.columns_count loop
   code_value:=prefix||'-R'||lpad(r::text,2,'0')||'-S'||lpad(c::text,3,'0');
   select id into keep_id
   from public.seats
   where zone_id=z.id and row_number=r and column_number=c and coalesce(is_aisle,false)=false
   order by case when status='booked'::public.seat_status then 0 when status='held'::public.seat_status then 1 else 2 end,id
   limit 1;
   ...
   if keep_id is null then
    insert into public.seats(
     ...
     z.organization_id,z.event_id,z.id,code_value,code_value,r,c,'available'::public.seat_status,
     section_value,seat_kind,coalesce(z.price_bhd,0),z.college_id,false,z.color,c,r
    );
   else
    update public.seats set
     code=code_value,label=code_value,row_number=r,column_number=c,
     section=section_value,seat_type=seat_kind,price_bhd=coalesce(z.price_bhd,0),
     college_id=z.college_id,is_aisle=false,color=z.color,x_position=c,y_position=r
    where id=keep_id;
   end if;
...
create index if not exists idx_people_arrival_status on public.people_directory(organization_id,arrival_status);
commit;
